    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex flex-col items-center mb-8 text-center">
          <div className="bg-slate-900/90 p-4 rounded-3xl border border-slate-800 shadow-2xl mb-4 inline-flex items-center justify-center">
            <MbrLogo className="w-36 h-auto" />
          </div>
        ...
        <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative">
          ...
          {/* Atalhos Rápidos de Teste para o Usuário */}
          <div className="mt-8 pt-6 border-t border-slate-800/80">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center mb-3 flex items-center justify-center gap-1.5">
              <KeyRound className="w-3.5 h-3.5 text-[#F26522]" /> Credenciais de Demonstração
            </p>
            
            <div className="grid grid-cols-2 gap-2.5">
              <button
                type="button"
                onClick={() => handleQuickFill('admin@mbrtracker.com.br', 'admin123')}
                className="p-2.5 bg-slate-950/80 hover:bg-slate-800 border border-slate-800/80 hover:border-slate-700 rounded-xl text-left transition-all cursor-pointer group"
              >
                <div className="flex items-center gap-1.5 text-xs font-black text-amber-400">
                  <ShieldCheck className="w-3.5 h-3.5 shrink-0" />
                  <span>Admin</span>
                </div>
                <p className="text-[10px] font-medium text-slate-400 mt-0.5 truncate">Gestão de Usuários</p>
              </button>

              <button
                type="button"
                onClick={() => handleQuickFill('usuario@mbrtracker.com.br', 'user123')}
                className="p-2.5 bg-slate-950/80 hover:bg-slate-800 border border-slate-800/80 hover:border-slate-700 rounded-xl text-left transition-all cursor-pointer group"
              >
                ...
                <p className="text-[10px] font-medium text-slate-400 mt-0.5 truncate">Painel Financeiro</p>
              </button>
            </div>
          </div>
